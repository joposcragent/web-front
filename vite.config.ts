...
export default defineConfig(({ mode }) => {
  ...
  const asyncJobsCrudTarget =
    env.VITE_DEV_PROXY_ORCHESTRATION_ASYNC_JOBS_CRUD ?? 'http://127.0.0.1:8085'
  ...
  return {
    ...
    server: {
      proxy: {
        ...
        '/orchestration-async-jobs-crud': { target: asyncJobsCrudTarget, changeOrigin: true },
