...
import {
  ...
  listAsyncJobs,
  listRelatedEntities,
  type AsyncJobsListParams,
} from "@/features/orchestrator/api/asyncJobsApi";
import { mapApiError } from "@/shared/api/mapApiError";
import {
  jsonPreview as previewJson,
  textPreview,
} from "@/shared/lib/jsonPreview";
import {
...
const STATUS_OPTS: AsyncJobStatusDto[] = [
  "STARTED",
  "SUCCEEDED",
  "FAILED",
  "CANCELED",
...
watch(
  ...
  async ([open, jobUuid]) => {
    ...
    const isStale = () => requestToken !== relatedRequestToken;
    if (!open || !jobUuid) {
    ...
    relatedData.value = null;
    try {
      const next = await listRelatedEntities(jobUuid);
      if (isStale()) return;
      relatedData.value = next;
    } catch (e) {
      if (isStale()) return;
      relatedError.value = mapApiError(e);
    } finally {
      if (!isStale()) {
        relatedLoading.value = false;
      }
...
watch(menuUuid, (open) => {
  if (open) draftJobUuid.value = filterJobUuid.value;
});
watch(menuParent, (open) => {
  if (open) draftParentUuid.value = filterParentJobUuid.value;
});
watch(menuStatus, (open) => {
  if (open) draftStatus.value = filterStatus.value;
});
...
async function loadList() {
  ...
  try {
    ...
    const params: AsyncJobsListParams = {
      page: page.value,
...
watch(
  ...
  (ju) => {
    ...
    if (filterJobUuid.value === ju && draftJobUuid.value === ju) return;
...
<template>
  <div>
    ...
    >
      ...
      <template
        #header.parentUuid="{ column, getSortIcon, toggleSort, isSorted }"
      >
        <div class="d-flex align-center ga-1 flex-wrap">
          <span>{{ column.title }}</span>
          <v-btn
      ...
      <template
        #header.started_at="{ column, getSortIcon, toggleSort, isSorted }"
      >
        <div class="d-flex align-center ga-1 flex-wrap">
          <span>{{ column.title }}</span>
          <v-btn
      ...
      <template
        #header.updated_at="{ column, getSortIcon, toggleSort, isSorted }"
      >
        <div class="d-flex align-center ga-1 flex-wrap">
          <span>{{ column.title }}</span>
          <v-btn
      ...
      <template
        #header.finished_at="{ column, getSortIcon, toggleSort, isSorted }"
      >
        <div class="d-flex align-center ga-1 flex-wrap">
          <span>{{ column.title }}</span>
          <v-btn
      ...
      <template #item.started_at="{ item }">
        <button
          ...
          role="link"
          class="cell-link text-left"
          :title="formatDisplayDateTimeFull(item.started_at)"
          @click="openDetail(item)"
        >
      ...
      <template #item.updated_at="{ item }">
        <button
          ...
          :title="
            item.updated_at
              ? formatDisplayDateTimeFull(item.updated_at)
              : undefined
          "
          @click="openDetail(item)"
        >
      ...
      <template #item.finished_at="{ item }">
        <button
          ...
          :title="
            item.finished_at
              ? formatDisplayDateTimeFull(item.finished_at)
              : undefined
          "
          @click="openDetail(item)"
        >
      ...
      <v-card v-if="detailItem">
        ...
        <v-card-text>
          ...
          <pre class="detail-pre">{{
            detailJsonBlock("context", detailItem.context)
          }}</pre>
          <pre class="detail-pre mt-4">{{
            detailItem.result != null && detailItem.result !== ""
              ? `result:\n${detailItem.result}`
              : "result: null"
          }}</pre>
          ...
          <template v-else-if="relatedData">
            ...
            <ul
              v-if="relatedData.jobPostingsList.length"
              class="related-uuid-list pl-4 mb-4"
            >
              <li
                v-for="id in relatedData.jobPostingsList"
                :key="`jp-${id}`"
                class="text-mono"
              >
                {{ id }}
            ...
            <ul
              v-if="relatedData.searchQueriesList.length"
              class="related-uuid-list pl-4 mb-0"
            >
              <li
                v-for="id in relatedData.searchQueriesList"
                :key="`sq-${id}`"
                class="text-mono"
              >
                {{ id }}
