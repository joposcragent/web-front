...
import {
  ...
  postEnqueueCollectionQuery,
} from "@/api/conductorEnqueue";
import type { SearchQueriesItem } from "@/api/types";
import {
  createSearchQuery,
  deleteSearchQuery,
  listSearchQueries,
  patchSearchQuery,
  type SearchQueryUpsertPayload,
} from "@/features/search-queries/api/searchQueriesApi";
...
async function submitDialog() {
  ...
  const nrErr = relevanceError(
    "Порог уведомлений",
    formNotificationRelevance.value,
  );
  if (crErr || nrErr) {
  ...
  try {
    if (dialogMode.value === "create") {
      ...
      const payload = currentFormPayload(name, query);
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          await createSearchQuery(uuid, payload);
          break;
        } catch (e: unknown) {
      ...
      if (orig) {
        ...
        if (
          !relEq(formNotificationRelevance.value, orig.notificationRelevance)
        ) {
          body.notificationRelevance = formNotificationRelevance.value;
        }
        if (formIsActive.value !== orig.isActive)
          body.isActive = formIsActive.value;
      ...
      await patchSearchQuery(editUuid.value, body);
    }
...
async function confirmDelete() {
  ...
  deleteError.value = null;
  try {
    await deleteSearchQuery(deleteTarget.value.uuid);
...
const tableItems = computed(() =>
  items.value.map((r) => ({
    ...
    dateDisplay: formatDisplayDateTime(
      r.updatedAt?.trim() || r.createdAt?.trim(),
    ),
  })),
);
...
function openEditByUuid(uuid: string) {
  const row = items.value.find((item) => item.uuid === uuid);
  if (row) {
    openEdit(row);
    return;
  }
  return loadList().then(() => {
    const freshRow = items.value.find((item) => item.uuid === uuid);
    if (freshRow) openEdit(freshRow);
  });
}
  ...
  <div>
    ...
    >
      ...
      <template #item.isLazyScraping="{ item }">
        <v-chip
          size="small"
          :color="item.isLazyScraping ? 'primary' : 'default'"
          variant="tonal"
        >
      ...
      <template #item.actions="{ item }">
        ...
        <v-btn color="error" variant="text" @click="askDelete(item)"
          >Удалить</v-btn
        >
      </template>
    </v-data-table>
    ...
    <v-dialog v-model="dialogOpen" max-width="560">
      <v-card>
        <v-card-title>{{
          dialogMode === "create" ? "Новый запрос" : "Изменение запроса"
        }}</v-card-title>
        <v-card-text>
          ...
          <v-checkbox
            ...
            label="Активен (участвует в плановом сборе)"
            density="comfortable"
          />
          <v-checkbox
